...
class Renungan_adm extends CI_Controller 
{
	...
	function Renungan_adm()
	{
		...
		//more
		$this->load->model('Renungan_model',     'renungan_model');
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('RENUNGAN.LIST');
	...
	**/
	function ajx_view($v=true)
	{
		...
		//view
		if(!$v)
		   $this->load->view('renungan.view.php',$vdata);
		else
		   echo $json_str;
	...
	**/
	function fmt_jason_data($plist=null, $page=1, $total=0, $draw=1)
	{
		...
		{
			$mhash    = u_encrypt_hash($vv->renungan_id);
			//edit
			$seq      = array('renungan_adm', 
					  'efrm', 
					  @rawurlencode( $mhash ) ,
					  @rawurlencode("$vv->renungan_id"),
					  );
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('RENUNGAN.ADD');
		...
		//view
		$this->load->view('renungan.add.frm.php',$vdata);
	...
	**/
	function afrm_proc()
	{
		...
		if(!$this->input->get_post('Save'))
		{
			...
			//fwd
			$this->load->view('renungan.add.frm.php',$pdata);
			return;            
		...
		//exec
		$pret                = $this->renungan_model->add($pdata);
		if(!$pret['status'])
		{
		...
		//fwd
		redirect(site_url("renungan_adm/view"));
	...
	**/	
	function set_rules_for_add()
	{
		...
		$this->form_validation->set_rules('name',     'Renungan Title',       'trim|required');
		$this->form_validation->set_rules('content',     'Renungan Content',       'trim|required');
	}
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('RENUNGAN.EDIT');
		...
		//get rec
		$gdata = $this->renungan_model->select_by_id( array('id' => $id) );
		...
		$vdata['jData_Hidden']      = array('id'=> $id, 'hash' => $hash, 'name' => $gdata['data']->renungan_title);
		//set data
		$vdata['jData_publish_list'] = $this->config->item('DEFAULT_PUBLISH_NOTPUBLISH_LIST');
		...
		//view
		$this->load->view('renungan.edit.frm.php',$vdata);
	...
	**/
	function efrm_proc()
	{
		...
		//upd8 it;-)
		$ddata = $this->renungan_model->update($pdata);
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('RENUNGAN.DELETE');
		...
		//view
		$this->load->view('renungan.delete.frm.php',$vdata);
	...
	**/
	function dfrm_proc()
	{
		...
		//delete it;-)
		$ddata = $this->renungan_model->delete(array(
								'id'         => $id,
...
/* End of file renungan.php */
/* Location: ./apps/controllers/renungan.php */