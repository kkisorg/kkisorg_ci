...
class Pengumuman_adm extends CI_Controller 
{

	function Pengumuman_adm()
	{
		parent::__construct();	
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('PENGUMUMAN.LIST');
	...
	**/
	function ajx_view($v=true)
	{
		...
		//role-list
		$rdata = $this->pengumuman_model->get(array(
						'order' => $fdata['order'],
						'limit' => $fdata['limit'],
		...
		//view
		if(!$v)
		   $this->load->view('pengumuman.view.php',$vdata);
		else
		   echo $json_str;
	...
	**/
	function fmt_filter()
	{
		...
		if(strlen($name)>=MIN_FILTER_LEN)
		{
			...
			$bfr[]  =" pengumuman_title like '%$name%' ";
		}	
	...
	**/
	function fmt_jason_data($plist=null, $page=1, $total=0, $draw=1)
	{
		...
		{
			$mhash    = u_encrypt_hash($vv->pengumuman_id);
			//edit
			$seq      = array('pengumuman_adm', 
					  'efrm', 
					  @rawurlencode( $mhash ) ,
					  @rawurlencode("$vv->pengumuman_id"),
					  );
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('PENGUMUMAN.ADD');
		...
		//view
		$this->load->view('pengumuman.add.frm.php',$vdata);
	...
	**/
	function afrm_proc()
	{
		...
		if(!$this->input->get_post('Save'))
		{
			...
			//fwd
			$this->load->view('pengumuman.add.frm.php',$pdata);
			return;            
		...
		//exec
		$pret                = $this->pengumuman_model->add($pdata);
		if(!$pret['status'])
		{
		...
		//fwd
		redirect(site_url("pengumuman_adm/view"));
	...
	**/	
	function set_rules_for_add()
	{
		...
		$this->form_validation->set_rules('name',     'Pengumuman Title',       'trim|required');
		$this->form_validation->set_rules('content',     'Pengumuman Content',       'trim|required');
	}
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('PENGUMUMAN.EDIT');
		...
		//get rec
		$gdata = $this->pengumuman_model->select_by_id( array('id' => $id) );
		...
		$vdata['jData_Hidden']      = array('id'=> $id, 'hash' => $hash, 'name' => $gdata['data']->pengumuman_title);
		//set data
		$vdata['jData_publish_list'] = $this->config->item('DEFAULT_PUBLISH_NOTPUBLISH_LIST');
		...
		//view
		$this->load->view('pengumuman.edit.frm.php',$vdata);
	...
	**/
	function efrm_proc()
	{
		...
		//upd8 it;-)
		$ddata = $this->pengumuman_model->update($pdata);
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('PENGUMUMAN.DELETE');
		...
		//view
		$this->load->view('pengumuman.delete.frm.php',$vdata);
	...
	**/
	function dfrm_proc()
	{
		...
		//delete it;-)
		$ddata = $this->pengumuman_model->delete(array(
								'id'         => $id,
...
/* Location: ./apps/controllers/pengumuman.php */