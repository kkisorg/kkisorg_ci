...
/*
|--------------------------------------------------------------------------
| @Filename: laporan_kegiatan_adm.php
|--------------------------------------------------------------------------
| @Desc    : Blog Adm
| @Date    : 2012-06-16
| @Version : 1.0
...
class Laporan_kegiatan_adm extends CI_Controller
{
	...
	function __construct()
	{
		...
		//more
		$this->load->model('laporan_kegiatan_model',     'laporan_kegiatan_model');
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('LAPORANKEGIATAN.LIST');
	...
	**/
	function ajx_view($v=true)
	{
		...
		//role-list
		$rdata = $this->laporan_kegiatan_model->get(array(
						'order' => $fdata['order'],
						'limit' => $fdata['limit'],
		...
		//view
		if(!$v)
		   $this->load->view('laporan_kegiatan.view.php',$vdata);
		else
		   echo $json_str;
	...
	**/
	function fmt_filter()
	{
		...
		if(strlen($name)>=MIN_FILTER_LEN)
		{
			...
			$bfr[]  =" laporan_kegiatan_title like '%$name%' ";
		}
	...
	**/
	function fmt_jason_data($plist=null, $page=1, $total=0, $draw=1)
	{
		...
		{
			$mhash    = u_encrypt_hash($vv->laporan_kegiatan_id);
			//edit
			$seq      = array('laporan_kegiatan_adm',
					  'efrm',
					  @rawurlencode( $mhash ) ,
					  @rawurlencode("$vv->laporan_kegiatan_id"),
					  );
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('LAPORANKEGIATAN.ADD');
		...
		//view
		$this->load->view('laporan_kegiatan.add.frm.php',$vdata);
	...
	**/
	function afrm_proc()
	{
		...
		if(!$this->input->get_post('Save'))
		{
			...
			//fwd
			$this->load->view('laporan_kegiatan.add.frm.php',$pdata);
			return;
		...
		//exec
		$pret                = $this->laporan_kegiatan_model->add($pdata);
		if(!$pret['status'])
		{
		...
		//fwd
		redirect(site_url("laporan_kegiatan_adm/view"));
	...
	**/
	function set_rules_for_add()
	{
		...
		$this->form_validation->set_rules('name',     'Laporan Kegiatan Title',       'trim|required');
		$this->form_validation->set_rules('content',     'Laporan Kegiatan Content',       'trim|required');
	}
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('LAPORANKEGIATAN.EDIT');
		...
		//get rec
		$gdata = $this->laporan_kegiatan_model->select_by_id( array('id' => $id) );
		...
		$vdata['jData_Hidden']      = array('id'=> $id, 'hash' => $hash, 'name' => $gdata['data']->laporan_kegiatan_title);
		//set data
		$vdata['jData_publish_list'] = $this->config->item('DEFAULT_PUBLISH_NOTPUBLISH_LIST');
		...
		//view
		$this->load->view('laporan_kegiatan.edit.frm.php',$vdata);
	...
	**/
	function efrm_proc()
	{
		...
		//upd8 it;-)
		$ddata = $this->laporan_kegiatan_model->update($pdata);
	...
	**/
	...
	{
		//perms
		$this->etc->check_permission('LAPORANKEGIATAN.DELETE');
		...
		//view
		$this->load->view('laporan_kegiatan.delete.frm.php',$vdata);
	...
	**/
	function dfrm_proc()
	{
		...
		//delete it;-)
		$ddata = $this->laporan_kegiatan_model->delete(array(
								'id'         => $id,
...
/* Location: ./apps/controllers/laporan_kegiatan.php */
